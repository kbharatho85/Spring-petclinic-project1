...
RUN wget https://mirrors.estointernet.in/apache/tomcat/tomcat-8/v8.5.66/bin/apache-tomcat-8.5.66.tar.gz
RUN tar -zxvf apache-tomcat-8.5.66.tar.gz
RUN mv apache-tomcat-8.5.66 tomcat8
WORKDIR /opt/tomcat8/webapps
RUN wget https://jenkins-shiva.s3.amazonaws.com/target/petclinic.war
